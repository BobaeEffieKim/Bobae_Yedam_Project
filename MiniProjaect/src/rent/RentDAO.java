...
public class RentDAO extends DAO {
	...
	public List<Rent> selectAllRentDate(String rent_date) {
		...
		try {
			...
			String sql = "SELECT * " + "FROM rent " + "WHERE rent_date BETWEEN "
					+ "(SELECT TO_TIMESTAMP(?, 'YYYY-MM-DD HH24:MI:SS') FROM dual) " + "AND "
	...
	public void updateReturnTime(Rent rent) {
		...
		try {
			...
			String sql = "UPDATE rent set return_time = sysdate " + "WHERE seat_num = ? AND member_id = ?";
	...
	public Rent selectOneById(String memberId) {
		...
		try {
			...
			while (rs.next()) {
				...
				rent.setSeatNum(rs.getInt("seat_num"));
				rent.setSeatPrice(rs.getInt("seat_price"));
				//rent.setSeatRental(rs.getInt("seat_rental"));
			}
	...
	public Rent selectOneByRental(int seatRental) {
		...
		try {
			...
			String sql = "SELECT seat_num FROM rents WHERE seatRental = ?";
	...
	// 단건조회
/*
	public Rent selectOneByMemeberRental(rent rent) {
...
//
//		public Rent rentCheck(String memberId) {
//			
//			Rent rent = null;
//			
//			try {
//				connect();
//				
//				String sql = "SELECT seat_rental FROM rent "
//						+ "WHERE seat_rental = 1"
//						+ "AND member_id = ? ";
//				pstmt = conn.prepareStatement(sql);
//				pstmt.setString(1, memberId);
